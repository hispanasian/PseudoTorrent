...
public class Message implements ProtocolMessage
{
	...
	{
		byte[] bytes = new byte[(payload.size() + 7) / 8];
	    for (int i=0; i < payload.length(); i++) 
	    {
	        if (payload.get(i)) bytes[bytes.length-i/8-1] |= 1<<(i%8);
	    } /* end loop */
	...
	/**
	 * Returns the payload as a BitSet. Credit to java2.com
	 * whose work can be found here:
	 * http://www.java2s.com/Code/Java/Language-Basics/ConvertingBetweenaBitSetandaByteArray.htm
	...
	protected BitSet payloadToBitSet()
	{
	    ...
	    for (int i = 0; i < this.payload.length * 8; i++) {
	      if ((this.payload[this.payload.length - i / 8 - 1] & (1 << (i % 8))) > 0) {
	        bits.set(i);
	      }
	    }
	    return bits;
